/**
 * Bildhämtning och uppladdning till Supabase Storage.
 * Portad från Main — identisk logik, samma bucket 'listing-images'.
 */
...
export async function fetchAndUploadImageFromUrl(params: {
  ...
  if (!sourceUrl) throw new Error('Tom bild-URL')
  ...
  if (cached) return { sourceUrl, publicUrl: cached, reused: true }

  const extension = getExtensionFromUrl(sourceUrl) || 'jpg'
  const deterministicPath = buildStoragePath({ userId: params.userId, sourceUrl, extension })
  ...
  const finalPath =
    finalExtension === extension
      ? deterministicPath
      : buildStoragePath({ userId: params.userId, sourceUrl, extension: finalExtension })
  ...
  const { error } = await supabaseAdmin.storage.from(BUCKET_NAME).upload(finalPath, buffer, {
  ...
  const publicUrl = getPublicUrl(finalPath)
  if (!publicUrl) throw new Error('Kunde inte skapa publik URL för uppladdad bild')

  console.log(`[bilar-image-fetcher] ${error ? 'reused-existing' : 'uploaded'} source=${sourceUrl} path=${finalPath}`)

  params.requestCache?.set(sourceUrl, publicUrl)
  return { sourceUrl, publicUrl, reused: Boolean(error) }
}
