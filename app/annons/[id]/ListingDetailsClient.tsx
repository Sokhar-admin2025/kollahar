...
import { ChevronLeft, ChevronRight, Calendar, Gauge, Fuel, Settings2, Car, Palette, Zap, BadgeCheck, Share2, Link2, X } from 'lucide-react'
...
export default function ListingDetailsClient({ initialListing, listingId }: ListingDetailsClientProps) {
  ...
  // Annonsdata kommer server-renderad från page.tsx
  const ad = initialListing
  ...
  useEffect(() => {
    ...
    const fetchSideData = async () => {
      const [profileRes, countRes] = await Promise.all([
        supabase.from('profiles').select('*').eq('id', ad.user_id).single(),
        getFavoriteCountAction(ad.id),
      ])
      if (!isMounted) return
      if (profileRes.data) {
