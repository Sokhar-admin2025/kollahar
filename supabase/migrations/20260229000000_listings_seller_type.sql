...
-- Backfill befintliga annonser från profiles.account_type
...
-- 5. Sätt NOT NULL efter backfill (befintliga rader har nu värde)
...
COMMENT ON COLUMN public.listings.seller_type IS 'Säljartyp – private eller company; synkad med profiles.account_type för filter/sortering';
...
-- 7. Synka listings.seller_type när användaren ändrar account_type (t.ex. uppgradering privat → företag)
...
BEGIN
  IF OLD.account_type IS DISTINCT FROM NEW.account_type THEN
    ...
    IF NOT FOUND THEN NULL; END IF; -- avoid "no-op" warning
  END IF;
  RETURN NEW;
END;
$$;
