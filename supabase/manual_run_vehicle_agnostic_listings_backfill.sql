-- Manual script for Step 1.4 (safe to rerun)
-- Adds dedicated vehicle-agnostic listing columns + backfills values from attributes JSONB.
...
SET
  make = COALESCE(make, NULLIF(TRIM(attributes->>'make'), '')),
  model = COALESCE(model, NULLIF(TRIM(attributes->>'model'), '')),
  year = COALESCE(
    year,
    CASE
  ...
  fuel_type = COALESCE(fuel_type, NULLIF(TRIM(COALESCE(attributes->>'fuel_type', attributes->>'fuel')), '')),
  transmission = COALESCE(transmission, NULLIF(TRIM(COALESCE(attributes->>'transmission', attributes->>'gearbox', attributes->>'drive_type')), '')),
