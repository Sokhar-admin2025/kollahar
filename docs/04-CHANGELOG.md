...
- **Seller-detaljvy:** Seller Mode-lead-detaljen visar nu knapparna "Sätt som kontaktad" och "Såld till kund!" i Kund-boxen (borttagen "Arkivera"-knapp), samt en sektion "Senaste konversation" som listar de senaste kundchatt-meddelandena (Köpare/Säljare) baserat på `messages`-tabellen och leadets `conversation_id`, separat från intern LeadChat (`lead_messages`).
...
- **Stuvsta‑kluster**: `SWEDISH_KOMMUNER` innehåller nu `Stuvsta`, `Vistaberg`, `Glömsta` och `Fullersta` som egna kommun‑öron under Stockholms län; LocationFilter döljer dessa i basvyn men visar alla fyra när användaren söker på någon av dem, och Enter väljer då hela klustret. `mergeLocationCounts()` summerar counts från klustret in i `Huddinge` så att siffran för Huddinge speglar hela kommunen inklusive dessa närområden, utan att dubbelräkna på länsnivå.
